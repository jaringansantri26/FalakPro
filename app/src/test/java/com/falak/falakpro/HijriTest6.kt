...
class HijriTest6 {
    ...
    fun testHijri() {
        ...
        if (f.exists()) {
            ElpDataProvider.initialize(f.inputStream())
        }
        val f2 = File("src/main/assets/earth_vsop87d.bin")
        if (f2.exists()) {
            Vsop87SolarEngine.initialize(f2.inputStream())
        }
        ...
        println("MABIMS TRUE CALCULATION:")
        println("Moon Topo Alt: $moonTopoAlt")
        println("Elongation: $elongGeo")
        println("Before Ghurub? ${ijtimaGeoJde < ghurubJde}")
        println("Is Visible function: ${HilalEngine.computeHilalVisibility(ijtimaGeoJde, lat, lon, elev, criteria, ghurubSunUt)}")
        ...
        println("Length of Month 11: ${startJde12 - startJde11}")
    }
}
